  ...
  useEffect(() => {
    const checkExchangeConnection = async () => {
      try {
        ...
        const response = await fetch(`https://13.60.210.111/api/user/${userId}/key-status`);
        ...
        if (response.ok) {
          ...
          if (data.registered) {
            ...
            try {
              const portfolioResponse = await fetch('https://13.60.210.111/api/proxy/binance', {
                method: 'POST',
                headers: {
        ...
        logger.error("Could not connect to proxy server at https://13.60.210.111");
      } finally {
        setIsLoading(false);
      }
    };
          ...
          <CardContent className="p-4">
            <div className="flex items-center text-amber-800 dark:text-amber-300">
              ...
              <div>
                ...
                <p className="text-sm mt-2">Make sure your proxy server is running at https://13.60.210.111</p>
              </div>
            </div>
          </CardContent>
        </Card>
