...
export function ConnectExchangeModal({ open, onOpenChange, userId, onSuccess }: ConnectExchangeModalProps) {
  ...
  useEffect(() => {
    ...
    const checkProxyServer = async () => {
      try {
        const response = await fetch('https://remedies-postal-travel-bailey.trycloudflare.com/health', { 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`Connecting to external backend at https://remedies-postal-travel-bailey.trycloudflare.com for user ${effectiveUserId}`);
      
      if (!proxyServerAvailable) {
        throw new Error("Cannot connect to proxy server at https://remedies-postal-travel-bailey.trycloudflare.com. Make sure it's running.");
      }
      
      // Send credentials directly to your backend server
      const backendResponse = await fetch('https://remedies-postal-travel-bailey.trycloudflare.com/api/register-key', {
        method: 'POST',
        headers: {
        ...
        {!proxyServerAvailable && (
          <Alert variant="destructive" className="mb-4 border-red-500 bg-red-50 dark:bg-red-900/20">
            ...
            <AlertDescription className="text-red-700 dark:text-red-400">
              Cannot connect to proxy server at https://remedies-postal-travel-bailey.trycloudflare.com. Please make sure it's running.
            </AlertDescription>
          </Alert>
        )}
